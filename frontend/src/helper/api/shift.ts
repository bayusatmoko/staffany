...
export const getShifts = async (week: Week) => {
  const { start, end } = week
  const api = getAxiosInstance()
  const { data } = await api.get(`/shifts?order[date]=DESC&order[startTime]=ASC&where[start]=${start}&where[end]=${end}`);
  return data;
};

export const publishWeek = async (id: string) => {
  const api = getAxiosInstance()
  const { data } = await api.post(`/weeks`, { weekId: id });
  return data;
};
